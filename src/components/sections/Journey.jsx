...
function JourneyCard({ item, index }) {
  ...
  return (
    <div className="flex flex-col items-center shrink-0" style={{ width: '380px' }}>
      ...
      <GlassCard
        className={`
          w-full p-6 border-2 transition-all duration-500 hover:-translate-y-2
          ${cfg.border} ${cfg.glow}
        `}
        hoverGlow
      >
        ...
        {/* Year */}
        <div className="font-accent text-4xl font-bold mb-3" style={{ color: cfg.accent }}>
          {item.year}
        </div>
        ...
        {/* Location */}
        {item.location && (
          <p className="text-xs font-code text-text-muted/60 mb-3">
            📍 {item.location}
          </p>
        )}
...
export default function Journey() {
  ...
  // Transform vertical scroll to horizontal translation with spring
  const xRaw = useTransform(scrollYProgress, [0, 1], [0, -scrollRange]);
  const x = useSpring(xRaw, { stiffness: 100, damping: 30, mass: 0.5 });

  // Calculate section height (3x viewport height for smooth scroll)
  const sectionHeight = scrollRange > 0 ? scrollRange + window.innerHeight : window.innerHeight * 3;
  ...
  return (
    ...
    >
      {/* Sticky container that moves horizontally */}
        ...
        >
          ...
          <div className="shrink-0 flex flex-col justify-center" style={{ width: '500px' }}>
            <span className="inline-flex items-center gap-2 text-[10px] font-code font-bold tracking-[0.3em] uppercase text-primary-glow-from mb-4">
              <span className="w-4 h-px bg-primary-glow-from/60" />
              Career Path
            </span>
          ...
          {/* End spacer with summary */}
          <div className="shrink-0 flex flex-col justify-center gap-6" style={{ width: '400px' }}>
            <div className="text-center">
              <div className="font-accent text-6xl font-bold text-primary-glow-from mb-2">
                {experience.length}
              </div>
              <div className="text-sm font-code text-text-muted uppercase tracking-widest">
                Key Milestones
              </div>
            </div>
